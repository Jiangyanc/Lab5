...
p = ggplot() +
  geom_line(data = samp,
            ...
            show.legend = FALSE) +                        # draw line for the data
  ...
  annotate("text",x=0.85,y=0.02,label="β",colour="black")+
  annotate("text",x=2.15,y=0.02,label="α",colour="white")+
  theme_classic()+
  theme(legend.position = c(0.15,1))+
  scale_color_manual(values = c("blue","red"))+
  scale_fill_manual(name="",values = c("blue","red"),breaks=c("x1","x2"),labels=c("Type Ⅰ error","Type Ⅱ error"))+
  labs(x=NULL,y=NULL)+
